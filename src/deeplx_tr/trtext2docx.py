...
async def main():  # pylint: disable=missing-function-docstring
    is_test = os.getenv("IS_TEST") or False

    if is_test:
        filepath = r"C:\syncthing\00xfer\2021it\2024-05-30.txt"
        texts = loadtext(filepath)[:10]  # for testing
    else:
        todayfile = rf"C:\syncthing\00xfer\2021it\{datetime.date.today()}.txt"
        if Path(todayfile).exists():
            filepath = todayfile
    ...
    if not is_test:
        then = monotonic()
        gpttext = await batch_newapi_tr(texts)
        gpttext_dict = dict(gpttext)
        col2 = [gpttext_dict.get(elm, "") for elm in range(n_paras)]
        console.print(f"done gpt-tr in {duration_human(monotonic() - then)}")
    ...
    if is_test:
        ...
        fname = Path(filepath).stem + "-tr.docx"

    ofile = (Path(filepath).parent / fname).as_posix()

    if is_test:
        trtext2docx(texts, col1, col1, ofile)
    else:
        trtext2docx(texts, col1, col2, ofile)
